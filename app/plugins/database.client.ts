...
export default defineNuxtPlugin(async () => {
  ...
  if (Capacitor.isNativePlatform()) {
    ...
    return { provide: { dbError: readonly(dbError) } }
  }
  ...
  worker.addEventListener('message', (e: MessageEvent) => {
    ...
    if ('type' in msg) return // lifecycle signals
  ...
  await new Promise<void>((resolve) => {
    ...
    worker?.addEventListener('message', function handler(e: MessageEvent) {
      ...
      if (type === 'READY') {
        ...
        clearTimeout(t)
        worker?.removeEventListener('message', handler)
        dbError.value = 'Hearth is already open in another tab. Close that tab and refresh.'
        resolve()
    ...
    if (worker) {
      worker.onerror = () => {
        clearTimeout(t)
        if (!dbError.value) dbError.value = 'Database failed to initialize. Try refreshing.'
        resolve()
      }
    }
  })
